...
laptop. Override the cache directory with the ``NOVEL_IDEA_CACHE`` environment
variable, or pass different model ids to the public API.
"""
...
# Disk cache for vocabulary embeddings. Override via the NOVEL_IDEA_CACHE env var.
CACHE_DIR = Path(
    os.environ.get(
        "NOVEL_IDEA_CACHE", Path.home() / ".cache" / "novel_idea_generator"
    )
)
